...
public class SignatureDetectionAbility implements CardAbility {
    @Override
    public void onCardDeployed(CardActor owner, CardActor deployedCard, String lane, GameplayScreen screen) {
        ...
        // Jika musuh meletakkan kartu di DMZ
        if (isOwnerPlayer != isDeployedPlayer && lane.equalsIgnoreCase("DMZ")) {
            int hp = CombatResolver.getHp(deployedCard.getData()) - 1;
            CombatResolver.setHp(deployedCard.getData(), hp);

            Gdx.app.log("Skill", "SIGNATURE DETECTION! IDS menembak jatuh target di DMZ.");
